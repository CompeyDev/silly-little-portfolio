...
<Layout leftAlign={true}>
    ...
    <br />
    Recently, I set forth attempting to learn lower level languages, mainly [Rust](https://rust-lang.org). After hours of confused stack overflow searches and debugging — I gave up; the borrowing checking system was just too confusing for me (for now...).
    ...
    <br />
    Furthermore, I noticed how the pointer abstraction system was really simplified, which lead to quite less cluttered raw pointer statements. Moving on, I realized rate-limiting was vital and therefore set it up too, with a [handy little library](https://github.com/JGLTechnologies/gin-rate-limit). Overall, the developer experience with Go was amazing, the best way I could describe it would be:
    <br />
